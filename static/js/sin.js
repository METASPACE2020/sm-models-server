...
function sin_render_fulltopic(data) {
  var res = [];
  for (var i=0; i < data.length; i+=1) {
    res.push("<tr><td>" + data[i][0] + "</td><td>" + data[i][1].toFixed(4) + "</td></tr>");
  }
  return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' + res.join('') + '</table>';
}

function sin_render_shorttopic(data) {
  var res = [];
  for (var i=0; i<10 && i < data.length; i+=1) {
    res.push(data[i][0]);
  }
  return res.join(' ');
}

function sin_render_doexp(data, id) {
  if (data) {
    return "<a class=\"btn btn-primary btn-sm\" rel=\"group\" href=\"/runexp/" + id + "\">Participate!</a>";
  } else {
    return "Experiment finished.";
  }
}

function sin_render_result(id, name) {
  // return name;
  return '<a href="/result/' + id +'">' + name + '</a>';
}

function sin_render_plsa(id, name) {
  // return name;
  return '<a href="/plsa/' + id +'">' + name + '</a>';
}

function sin_render_exptype(id, name) {
  return name;
  // return '<a href="/exptype/' + id +'">' + name + '</a>';
}
